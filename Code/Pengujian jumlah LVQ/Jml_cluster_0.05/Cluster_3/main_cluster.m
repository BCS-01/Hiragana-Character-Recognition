...
for i = 1 : length(srcFiles)
    filename = strcat('D:\Kuliah\Semester 8\Tugas Akhir\Gambar\target\',srcFiles(1).name);
...
cluster_3=percobaan(urutan(1),:); %mengambil cluster yang memiliki nilai terbesar

%simpan cluster
templvq1=cluster_3{1,1}';
for i=1:size(cluster_3{1,1}',1)
    z=find(templvq1(i,1)==Simpan(:,197));
    LVQ1(i,:)=Simpan(z,:);
end

templvq2=cluster_3{1,2}';
for i=1:size(cluster_3{1,2}',1)
    z=find(templvq2(i,1)==Simpan(:,197));
    LVQ2(i,:)=Simpan(z,:);
end

templvq3=cluster_3{1,3}';
for i=1:size(cluster_3{1,3}',1)
    z=find(templvq3(i,1)==Simpan(:,197));
    LVQ3(i,:)=Simpan(z,:);
end

save Target_cluster_3.mat LVQ1 LVQ2 LVQ3 percobaan nilai urutan Simpan
