...
for i = 1 : length(srcFiles)
    filename = strcat('E:\Tugas Akhir\Gambar\target\',srcFiles(i).name);
...
jmlpercobaan=8000; %jumlah percobaan kmeans
n=8; %brp cluster
...
for i=1:size(cluster_7{1,6}',1)
    z=find(templvq6(i,1)==Simpan(:,197));
    LVQ6(i,:)=Simpan(z,:);
end

templvq7=cluster_7{1,7}';
for i=1:size(cluster_7{1,7}',1)
    z=find(templvq7(i,1)==Simpan(:,197));
    LVQ7(i,:)=Simpan(z,:);
end

templvq8=cluster_7{1,8}';
for i=1:size(cluster_7{1,8}',1)
    z=find(templvq8(i,1)==Simpan(:,197));
    LVQ8(i,:)=Simpan(z,:);
end

save Target_cluster_7.mat LVQ1 LVQ2 LVQ3 LVQ4 LVQ5 LVQ6 LVQ7 LVQ8