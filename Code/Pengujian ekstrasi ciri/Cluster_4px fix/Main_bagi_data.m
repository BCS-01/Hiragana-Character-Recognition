...
for jj=1:46
    label=jj;
    baris_label=find(data_ekstrasi(:,101)==label); %mencari ciri dgn label
    ...
    barislatih=1;
    baristest=1;
    latih_tmp=zeros(set_latih,101);
    test=zeros(jml_t-set_latih,101);
    ...
    barislatih=1;
    barisvalidasi=1;
    latih=zeros(data_latih1-data_validasi,101);
    validasi=zeros(data_validasi,101);
    for j=1:data_latih1
