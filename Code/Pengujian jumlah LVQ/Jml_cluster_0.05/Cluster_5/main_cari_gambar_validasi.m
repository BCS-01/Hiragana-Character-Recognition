...
load bagi_data.mat validasi_set
...
jmlslh=1;
    for i=1:length(data(:,1))
        ...
        jmlgbr=size(validasi_set(:,1));
        for j=1:jmlgbr %baris = banyaknya gambar 
            for jj=1:baris
                if data_ekstrasitemp(jj,:)==validasi_set(j,:)
                    nama_validasi_set{jmlslh,1}=strcat(data{i,1},datatemp(jj,1).name);
                    jmlslh=jmlslh+1;
                end
            end
        end       
    end
for i=1:size(nama_validasi_set,1)
    copyfile(nama_validasi_set{i,1},'E:\ubah\program\gambar_validasi')
end

%save pre_ekstrasi_ciri.mat data_ekstrasi
save nama_validasi_set.mat nama_validasi_set