...
jmlslh=1;
    for i=1:length(data(:,1))
        ...
        jmlgbr=size(latih_set(:,1));
        for j=1:jmlgbr %baris = banyaknya gambar 
            for jj=1:baris
                if data_ekstrasitemp(jj,:)==latih_set(j,:)
                    nama_latih_set{jmlslh,1}=strcat(data{i,1},datatemp(jj,1).name);
                    jmlslh=jmlslh+1;
                end
            end
        end       
    end
for i=1:size(nama_latih_set,1)
    copyfile(nama_latih_set{i,1},'E:\ubah\program\gambar_latih')
end

%save pre_ekstrasi_ciri.mat data_ekstrasi
save nama_latih_set.mat nama_latih_set